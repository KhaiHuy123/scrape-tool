...
class ScrapeOpsFakeUserAgentMiddleware:
    def __init__(self, settings):
        ...
        self.scrapeops_endpoint = settings.get('SCRAPROPS_FAKE_USER_AGENT_ENDPOINT')
        self.scrapeops_fake_user_agent_active = settings.get('SCRAPEOPS_FAKE_USER_AGENT_ENABLED')
    ...
    def _get_user_agent_list(self):
        pay_load = {'api_key' : self.scrapeops_api_key}
        if self.scrapeops_num_results is not None :
            pay_load['num_results'] = self.scrapeops_num_results
        response = requests.get(self.scrapeops_endpoint, params= urlencode(pay_load))
    ...
    def _scrapeops_fake_user_agent_enabled(self):
        ...
        else: self.scrapeops_fake_user_agent_active = True
...
class ScrapeOpsFakeBrowserHeaderAgentMiddleware :
    def __init__(self, settings):
        self.scrapeops_api_key = settings.get('SCRAPEOPS_API_KEY')
        self.scrapeops_endpoint = settings.get('SCRAPROPS_FAKE_BROWSER_HEADER_ENDPOINT')
    ...
    def _scrapeops_fake_browser_headers_enabled(self):
        ...
        else :self.scrapeops_fake_browser_headers_active =True
...
class RotateProxyMiddleware__Incompleted:
    def __init__(self, settings):
    ...
    def process_request(self, request, spider):
        ...
        if request.meta.get('proxy') is not None:
            print('----------- PROXY INFO -----------')
            print('Connected to proxy: {}'.format(host))
            print('Sucessfully connected !')
        else: print('Proxy not set for request.')
        pass

class RotateProxyMiddleware:
    def __init__(self, settings):
        self.proxy_params = settings.get('SCRAPERAPI_PROXY_PARAMS')
    ...
    def process_request(self, request, spider):
        random_params = random.choice(self.proxy_params)
        user_name, end_point, api_key = random_params
        try :
            proxy_url = f"http://{user_name}:{api_key}@{end_point}"
            request.meta['proxy'] = proxy_url
            if request.meta.get('proxy') is not None:
                print('----------- PROXY INFO -----------')
                print('Connected to proxy: {}'.format(end_point))
                print('Using API key: {}'.format(api_key))
                print('Successfully connected!')
            else: print('Proxy not set for request.')
        except :
            proxy_url = f'https ://{user_name}@{end_point}'
            request.meta['proxy'] = proxy_url
            if request.meta.get('proxy') is not None:
                print('----------- PROXY INFO -----------')
                print('Connected to proxy: {}'.format(end_point))
